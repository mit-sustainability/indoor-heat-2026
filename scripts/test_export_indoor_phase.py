...
def test_run_phase_manifest_points_to_correct_browser_base(tmp_path, monkeypatch):
    ...
    # No metadata.json was placed in the output dir, so the manifest must not
    # advertise one — otherwise the frontend would fetch a 404 and fail the load.
    assert "metadata" not in manifest["files"]


def test_run_phase_manifest_references_metadata_only_when_file_exists(tmp_path, monkeypatch):
    monkeypatch.setitem(PHASES, "phase1", {**PHASES["phase1"], "output_dir": str(tmp_path)})
